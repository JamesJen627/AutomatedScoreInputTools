...
| 800m得分占比       | √    | Number  | 20       |
| 50m成绩（单位：秒）    | △    | Number/String  | 7.58 或 8"7 |
| 50m得分占比        | √    | Number  | 20       |
...
1. **规范表头**：上表「字段名称」列，用于编辑器默认表头与导出。
2. **简写别名**：见 §5.21 `HEADER_ALIASES`；导入时自动映射为规范表头。
3. **中间得分列**：如 `800m得分`、`总得分` 等允许出现在输入文件中，导入时忽略，不参与必填校验。

导入时按**表头名称**映射列，不依赖固定列序号。

---

# 5.4 输出 Excel 文件规范

导出时保留原始输入字段，并按固定顺序**交错插入**得分列。

**共 22 列**，顺序如下（与 `00_DEMAND` §2.2 一致）：

1. 班级、考号、学号、姓名、性别
2. 坐位体前屈成绩（单位：厘米）→ 坐位体前屈得分 → 坐位体前屈得分占比
3. 800m成绩（单位：分·秒）→ 800m得分 → 800m得分占比
4. 50m成绩（单位：秒）→ 50m得分 → 50m得分占比
5. 立定跳远成绩（单位：米）→ 立定跳远得分 → 立定跳远得分占比
6. 仰卧起坐成绩（**导出列名无单位后缀**）→ 仰卧起坐得分 → 仰卧起坐得分占比
7. 总成绩
8. **等级（优≥90%/良≥80%/合≥60%）** — 系统计算，不可作为输入必填项
...
| totalScoreGrade   | Enum（优/良/合/不合格） |
| auditStatus       | Enum     |
| calculateTime     | DateTime |
| success           | Boolean  |
| errorMessage      | String（计算失败时） |
...
**基本信息与占比**：禁止为空；若为空 → Error。

**五项成绩字段**：

- 允许为空（缺考 / 未测）。
- 校验阶段：空值不报错。
- 计算阶段：空值视为 NaN，该项 **itemScore = 0**，**contributionScore = 0**，整体 `success = true`（除非缺少评分标准等其他错误）。

非空成绩若格式非法 → Error。

不得对空成绩自动补零填入 Excel；仅在计算结果中体现 0 分。
...
配置位置：`src/shared/constants/field-mapping.ts` → `STUDENT_FIELD_MAPPINGS`。

---

## ①-b 导入表头别名（HEADER_ALIASES）

以下简写表头在**导入**时自动映射为规范表头：
...
**导入忽略列**（可存在于输入 Excel，不参与校验）：各项目「得分」列、`总得分`、等级列。

---

## ①-c 导出列绑定（EXPORT_COLUMN_BINDINGS）

导出 22 列由 `EXPORT_COLUMN_BINDINGS` 声明：`input`（原输入）、`score`（系统计算得分）、`grade`（等级）三类列交错排列。详见 `00_DEMAND` §2.2。
